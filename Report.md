...
The images show the speedup of the different sorting algorithms across a varying number of processes under the computation large model. Here we can see that parallel Radix Sort the highest computation speedup as the # of processes increases, while the other algorithms remain relatively stable. Right after Radix sort, SampleSort shows the best performance as the speedup also increases with the number of processors. This suggests that these algorithms perform particularly well for high process counts and benefit from parallelism.
...
### Sample Sort(Eyad Nazir):
The Sample Sort algorithm performed as expected during the tests. The speedup increased consistently with the number of processes, particularly for larger matrix sizes such as 2<sup>28</sup>. This indicates that the algorithm scales well with the problem size and the number of processors. The strong scaling results showed effective performance up to 128 processes, with minimal communication overhead. However, beyond 128 processes, the communication overhead in the `comm_large` region became significant, causing the total time to increase, especially for larger matrices. This suggests that while the algorithm is efficient for a moderate number of processes, further optimization may be needed to handle higher process counts effectively. Overall, the implementation was successful, and the algorithm demonstrated good scalability and performance for the tested input sizes.

### Radix Sort(Eduardo Alvarez):
The performance of our parallel Radix Sort algorithm largely met expectations, especially for larger input sizes where the workload per processor was significant enough to justify parallelism. For the largest input sizes (e.g., 2^28), the algorithm demonstrated good scalability up to around 128 processors, with communication optimizations becoming effective at even higher processor counts. However, we observed some issues with smaller input sizes, such as 2^18, where communication overhead quickly outweighed the computational benefits, leading to diminishing returns and even a spike in overhead as the number of processors grew. The algorithm showed an early spike in communication time for smaller inputs, especially with disordered data. This suggests that our current communication strategy may not be optimized for smaller workloads or more irregular data patterns, which could be improved by refining data partitioning or minimizing synchronization points. To address these issues, further optimization is necessary, especially in handling communication for smaller input sizes and improving data distribution strategies for disordered inputs. Implementing asynchronous communication or dynamic load balancing might also help in reducing bottlenecks and achieving more consistent scaling across varying input types.

### Bitonic Sort(Juan Carrasco):
The performance analysis of my Bitonic Sorting algorithm shows good behavior as the number of processes increases, especially for input sizes larger than 2<sup>20</sup>. The average computation time decreases almost quadratically with the number of processors. For input size (2<sup>16</sup>), there is a spike in computation large time at 512 processors, which indicates for smaller input sizes, the overhead of MPI communication outweights the benefit of splitting the workload evenly amongst more processors. Another important thing to highlight is with regards to the Speedup performance compared to the other algorithms. Since Bitonic is a parallel algorithm, I expected it to perform better than the other algorithms, but this is not exactly the case when we analyze the Algorithm Performance Comparison Section. For Bitonic Sort, the computation speedup remains almost flat, which indicates that my parallel implementation of the algorithm does not scale well with number of processors. This issue could be because of how I handle the data exchange and merging between partners in my `comp_exchange` functions. I merge data between the processors, and then perform a local sort, which could be improved by merging the data and sorting it in one go. Overall, the implementation was successful, and the algorithm demonstrated good scalability and performance for the tested input sizes.
